...
class EcoflowPublicApiClient(EcoflowApiClient):
    ...
    def configure_device(self, device_data: DeviceData, api_devices_info: dict[str, Any] | None = None):
        ...
        from custom_components.ecoflow_cloud_resilient.devices.registry import device_by_product
    ...
    async def quota_all(self, device_sn: str | None):
        ...
        # quota/all may return cached telemetry for devices that are already
        # offline, so fetch the authoritative online state separately.
        online_by_sn: dict[str, bool] = {}
        try:
            for device in await self.fetch_all_available_devices():
        ...
        for sn in target_devices:
            try:
                ...
                is_online = online_by_sn.get(sn)
                params = {"params": raw["data"]} if raw.get("data") and is_online else None
                self.devices[sn].data.add_data(PreparedData(is_online, params, raw, False))
            except Exception as exception:
